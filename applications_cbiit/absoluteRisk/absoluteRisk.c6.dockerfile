...
LABEL \
  BASE_OS="CentOS 6.8" \
  DEFAULT_IMAGE="cbiit/absoluterisk" \
  DEFAULT_TAG="centos6" \
  DESCRIPTION="Deployment environment for Absolute Risk Calculator (based on CentOS 6.8)" \
  VERSION="1.0" \
  UID="ABSOLUTERISK_1.0_C6"
...
RUN pip install --upgrade \
 ...
 && R -e "install.packages('https://nciws-d709-v.nci.nih.gov/cbiit-package/cbiit/packages/iCARE_1.1.0.tar.gz', repos = NULL)"
